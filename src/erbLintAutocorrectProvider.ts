...
export class ERBLintAutocorrectProvider implements vscode.DocumentFormattingEditProvider {
  public provideDocumentFormattingEdits(document: vscode.TextDocument): vscode.TextEdit[] {
    ...
    try {
      ...
      if (config.executePath.length === 0) {
        stdout = execSync(`${config.command} ${args.join(' ')}`, options)
      } else {
        stdout = execFileSync(config.command, args, options)
      }
  ...
  private onSuccess(document: vscode.TextDocument, stdout: Buffer): vscode.TextEdit[] {
    ...
    return [vscode.TextEdit.replace(this.getFullRange(document), autoCorrected)]
  }
